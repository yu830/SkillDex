...
3. Keep claims evidence-based. If a link is not confirmed, leave it as an empty string.
...
4. Leave `repo`, `demo`, or `docs` blank until the link is real.

## Current Scope

- Static Vite + React + TypeScript site.
- Search across names, summaries, tags, and tools.
- Filters for category, tag, risk level, and compatible tool.
- Skill and Project card sections.
- Conservative visual style using Yu Serif/system serif fallbacks.

## Deployment

The project can be deployed to Vercel or GitHub Pages after a GitHub remote is configured. This local repository currently has no remote configured, so no push or hosted URL is claimed in Phase 1.
